...
class ApiSecurityManager:
    ...
    def update_api(self, host_and_base_path, api_spec, api_spec_id):
        ...
        try:
            response = requests.post(self.config.MANAGEMENT_URL + self.config.SITE_ID + "/" + str(api_spec_id) + ("?api_id=%s&api_key=%s" %(self.config.API_ID, self.config.API_KEY)), files=dict(fileContent=json.dumps(api_spec), validateHost=False), timeout=10)
            if response.status_code != 200:
    ...
    def upload_api(self, host_and_base_path, api_spec):
        ...
        try:
            response = requests.post(self.config.MANAGEMENT_URL + self.config.SITE_ID + ("?api_id=%s&api_key=%s" %(self.config.API_ID, self.config.API_KEY)), files=dict(fileContent=json.dumps(api_spec), validateHost=False), timeout=10)
            if response.status_code != 200:
